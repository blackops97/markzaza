...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == "admins" or matches[1] == "الادمنية" then
      ...
      return reply_msg(msg.id,'➖➖➖➖➖➖➖➖➖➖➖\n <b> group link🚀 : </b>  \n '..group_link..' \n ➖➖➖➖➖➖➖➖➖➖➖', ok_cb, false)
		...
		if matches[1] == 'clean' or  matches[1] == 'مسح' then
			...
			if matches[2] == 'mutelist' or  matches[1] == 'الادمنية' then
				chat_id = msg.to.id
		...
		if matches[1] == 'k' or matches[1] =='قفل' and is_momod(msg) then
			...
			if matches[2] == 'fwd'or matches[2] == 'اعادة توجيه' then
		...
		if matches[1] == 'n' or matches[1] =='فتح' and is_momod(msg) then
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return reply_msg(msg.id,'<b>Mute</b><b>'..msg_type..'</b><b>》has been disabled</b>',ok_cb, false)
				else
		...
		if matches[1] == "sal" or matches[1] == 'كتم' and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "sal" or matches[1] == 'كتم' and matches[2] and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "sal" or matches[1] == 'كتم'  and matches[2] and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
